...
namespace eft_dma_radar.Tarkov
{
    ...
    public sealed class MemDMA : MemDMABase
    {
        ...
        private readonly string[] _altProcessNames = { "EscapeFromTarkov_BE", "BsgLauncher" }; // 添加備用進程名
        ...
        private void MemoryPrimaryWorker()
        {
            ...
            // 新增初始化緩衝時間 (重要!)
            LoneLogging.WriteLine("Waiting for Unity engine initialization...");
            Thread.Sleep(2000);
        ...
        {
            const int maxRetries = 15; // 增加重試次數
            const int retryDelay = 300; // 延長重試間隔

            VmmProcess tmpProcess = null;
            ...
            for (int i = 0; i < maxRetries; i++)
            {
                ...
                {
                    // 嘗試主要進程名
                    tmpProcess = _hVMM.Process(_processName);
                    if (tmpProcess != null)
                    {
                        Process = tmpProcess;
                        LoneLogging.WriteLine($"[DMA] Process found: {_processName}");
                        return; // 成功找到進程
                    }

                    // 嘗試備用進程名
                    foreach (var altName in _altProcessNames)
                    {
                        tmpProcess = _hVMM.Process(altName);
                        if (tmpProcess != null)
                        {
                            Process = tmpProcess;
                            LoneLogging.WriteLine($"[DMA] Process found (alt): {altName}");
                            return; // 成功找到進程
                        }
                    }
                }
                ...
                {
                    /* 忽略單次查詢異常 */
                    LoneLogging.WriteLine($"[DMA] Process query attempt {i + 1} failed: {ex.Message}");
                }

                Thread.Sleep(retryDelay);
            }

            LoneLogging.WriteLine($"[DMA] Unable to find process after {maxRetries} attempts");
            throw new Exception($"Unable to find game process");
        }
        ...
        private void LoadModules()
        {
            ...
            LoneLogging.WriteLine($"[DMA] Modules loaded: Unity=0x{unityBase:X}, Mono=0x{monoBase:X}");
        }
        ...
        {
            const int maxRetries = 20; // 顯著增加重試次數
            const int retryDelay = 150; // 縮短重試間隔
            ...
            for (var i = 0; i < maxRetries; i++)
            {
                ...
                {
                    /* 忽略單次異常 */
                    LoneLogging.WriteLine($"[DMA] Process check attempt {i + 1} failed");
                }

                Thread.Sleep(retryDelay);
            }

            LoneLogging.WriteLine($"[DMA] Game process not found after {maxRetries} attempts");
            throw new GameNotRunning("Temporary DMA communication failure");
        }
